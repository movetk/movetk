...
struct DouglasPeuckerTests {
    ...
    using FindFarthest = movetk_algorithms::FindFarthest<MovetkGeometryKernel, Norm>;
    using DouglasPeucker = movetk_algorithms::DouglasPeucker<MovetkGeometryKernel, FindFarthest>;
};


MOVETK_TEMPLATE_LIST_TEST_CASE_METHOD(DouglasPeuckerTests,"Check Douglas-Peucker Simplification 1", "[douglas_peucker_simplification_1]")
{
...
MOVETK_TEMPLATE_LIST_TEST_CASE_METHOD(DouglasPeuckerTests, "Check Douglas-Peucker Simplification 2", "[douglas_peucker_simplification_2]")
{
    PolyLine polyline({make_point({-6.19, -3.46}), make_point({-4.99, 1.16}),
                       make_point({-2.79, -2.22}), make_point({-1.87, 0.58}),
                       make_point({0.77, 0.22}), make_point({-1.15, 3.06}),
...
MOVETK_TEMPLATE_LIST_TEST_CASE_METHOD(DouglasPeuckerTests, "Check Douglas-Peucker Simplification 3", "[douglas_peucker_simplification_3]")
{
    PolyLine polyline({make_point({-6.19, -3.46}), make_point({-4.99, 1.16}),
                       make_point({-2.79, -2.22}), make_point({-1.87, 0.58}),
                       make_point({0.77, 0.22}), make_point({-1.15, 3.06}),
...
MOVETK_TEMPLATE_LIST_TEST_CASE_METHOD(DouglasPeuckerTests, "Check Douglas-Peucker Simplification 4", "[douglas_peucker_simplification_4]")
{
