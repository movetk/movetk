...
TEST_CASE("brownian bridge model segmentation 1","[test brownian bridge model segmentation 1]"){
    ...
    movetk_algorithms::brownian_bridge::ParameterSelector<MovetkGeometryKernel, ParameterTraits> selector(5);
    ...
    typedef movetk_algorithms::brownian_bridge::LogLikelihood<MovetkGeometryKernel,
            ParameterTraits, Norm> LogLikelihood;
    ...
    segments.clear();
    segmentation1(std::cbegin(bridges), std::cend(bridges), std::cbegin(selected_coeffs), std::cend(selected_coeffs),
                  movetk_core::movetk_back_insert_iterator(segments));
    ...
    movetk_core::SegmentIdGenerator make_segment1(std::begin(segments), std::end(segments));
    std::cout<<"Segments: \n";
    bit = std::begin(bridges);
    id = 0;
    while (bit != std::end(bridges)){
