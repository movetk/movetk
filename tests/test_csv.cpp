...
TEST_CASE( "CSV is read", "[csv]" ) {
    ...
    SECTION( "CSV is read" ) {
        ...
        csv<Row, 0, 1, 2, 3, 4> table(in, ',', false);
    ...
    SECTION( "CSV with header is read" ) {
        ...
        std::tuple<std::string, std::string, std::string, std::string, std::string> expected_cols = {"col1", "col2", "col3", "col4", "col5"};
        std::stringstream in(
        ...
        csv<Row, 0, 1, 2, 3, 4> table(in, ',', true);
