...
TEST_CASE("Find minimum non zero element", "[test_min_non_zero_element]") {
	std::vector<typename MovetkGeometryKernel::NT> values{3.1, 2.2, 0, 1};
...
TEST_CASE("Calculate Euclidean distance", "[test_calculate_distance]") {
	std::array<typename MovetkGeometryKernel::NT, dimensions> p;
	std::vector<typename MovetkGeometryKernel::MovetkPoint> points;
	std::vector<typename MovetkGeometryKernel::NT> result;
...
TEST_CASE("Calculate Heading", "[test_get_headings]") {
	...
	movetk::utils::MakePoint<MovetkGeometryKernel> make_point;
	vector<NT> result;
...
TEST_CASE("Calculate Velocity", "[test_get_velocities]") {
	...
	movetk_core::get_velocities<MovetkGeometryKernel>(cbegin(speed),
	                                                  cend(speed),
	                                                  cbegin(heading),
...
TEST_CASE("Merge Intervals 2", "[test_merge_intervals_2]") {
	...
	auto beyond = movetk_core::merge_intervals<MovetkGeometryKernel>(std::begin(items), std::end(items));
	auto it = std::begin(items);
	REQUIRE(std::distance(it, beyond) == expected.size());
	auto eit = std::begin(expected);
	while (it != beyond) {
...
TEST_CASE("Compute Curve Length", "[compute_curve_length]") {
	...
	movetk_core::MakePoint<MovetkGeometryKernel> make_point;
	std::vector<MovetkGeometryKernel::MovetkPoint> polyline{make_point({2, 2}),
	                                                        make_point({4, 6}),
	                                                        make_point({8, 6}),
	                                                        make_point({6, 4}),
	                                                        make_point({8, 4}),
	...
	std::copy(std::begin(result), std::end(result), std::ostream_iterator<MovetkGeometryKernel::NT>(std::cout, "\n"));
}