...
MOVETK_TEMPLATE_LIST_TEST_CASE_METHOD(FreeSpaceDiagramTests, "Check free space cells", "[free_space_cell_1]") {
    ...
    for (const auto& test_case : test_cases) {
        SECTION(std::to_string(test_case_num)) {
            ...
            REQUIRE(test_helpers::range_has_same_value(fsc.vertices_begin(), fsc.vertices_end(),
                test_case.expected_free_vertices.begin(), test_case.expected_free_vertices.end()));
        }
        ++test_case_num;
    }
}
