...
public:
     ...
     */
    const std::vector<std::tuple<fields...>> &data() {
        return _points;
    }
    ...
    typename std::vector<std::tuple<fields...>>::reverse_iterator rbegin() {
        return _points.rbegin();
    }

    typename std::vector<std::tuple<fields...>>::reverse_iterator rend() {
        return _points.rend();
    }
...
//                    print_tuple(std::cout, *first);
    ...
    public:
        ...
        inline FieldIterator() : _parent(nullptr) {}

        /// Construct an iterator at the beginning of the @p parent object.
        inline FieldIterator(TabularTrajectory &parent) : _parent(&parent) {
            _it = _parent->begin();
        }

        inline TrajectoryIterator &underlying_iterator() {
            return _it;
        }

        inline const TrajectoryIterator &underlying_iterator() const {
            return _it;
        }
    ...
    FieldIterator<field_idx> end() {
        return FieldIterator<field_idx>();
    }
...
std::tuple<typename std::vector<fields>::iterator...>
field_iterators_increment(std::tuple<typename std::vector<fields>::iterator...> &iters, std::index_sequence<idx...>) {
    return {(std::get<idx>(iters)++)...};
}

template<class... fields>
std::tuple<typename std::vector<fields>::iterator...>
row_next(std::tuple<typename std::vector<fields>::iterator...> &iters) {
    return field_iterators_increment<fields...>(iters, std::make_index_sequence<sizeof...(fields)>{});
}

template<class... fields, std::size_t... idx>
typename std::tuple<fields...>
_read_row(std::tuple<typename std::vector<fields>::iterator...> &iters, std::index_sequence<idx...>) {
    return std::make_tuple(*std::get<idx>(iters)...);
}

template<class... fields>
typename std::tuple<fields...>
read_row(std::tuple<typename std::vector<fields>::iterator...> &iters) {
    return _read_row<fields...>(iters, std::make_index_sequence<sizeof...(fields)>{});
}

template<class... fields, class... new_fields>
TabularTrajectory<fields..., new_fields...>
concat_field(TabularTrajectory<fields...> t, std::vector<new_fields>... new_columns) {
