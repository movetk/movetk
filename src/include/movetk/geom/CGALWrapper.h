...
namespace movetk_support {
    ...
    namespace cgal {
        template<template<typename...> typename Predicate, typename Arg, typename Tuple>
        struct predicate_conjunction : public std::false_type {};
        template<template<typename...> typename Predicate, typename Arg, typename...Ts>
        struct predicate_conjunction<Predicate, Arg, std::tuple<Ts...>>
        {
            static constexpr bool value = std::conjunction_v<Predicate<Args, Ts>...>;
        };
        template<template<typename...> typename Predicate, typename Arg, typename Tuple>
        struct predicate_disjunction : public std::false_type {};
        template<template<typename...> typename Predicate, typename Arg, typename...Ts>
        struct predicate_disjunction<Predicate, Arg, std::tuple<Ts...>>
        {
            static constexpr bool value = std::disjunction_v<Predicate<Args, Ts>...>;
        };

        template<typename Type>
        struct is_valid_NT {
            using CGAL_types = std::tuple<long double, CGAL::MP_Float, CGAL::Gmpfr, CGAL::Gmpq, CGAL::Mpzf>;
            static constexpr bool value = predicate_disjunction<std::is_same, Type, CGAL_types>;
        };
    }
    ...
    */
    template<class Type>
    struct is_valid_NT {
        static constexpr bool value = cgal::is_valid_NT<Type>::value;
        static constexpr bool can_cast_to_string = std::is_same_v<Type, long double>;
    };
    ...
    public:
        ...
        Wrapper_CGAL_Point(const Wrapper_CGAL_Point& p) = default;
        Wrapper_CGAL_Point(Wrapper_CGAL_Point&& p) = default;
    ...
    */
    ...
    public:
        ...
        const Wrapper_Point& operator[](size_t idx) const {
            return line[idx];
        }
