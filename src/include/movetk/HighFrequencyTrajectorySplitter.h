...
public:
    using trajectory_type = typename std::iterator_traits<TrajectoryInputIterator>::value_type;  // TabularTrajectory<fields...>
    using ProbePoint = typename trajectory_type::value_type;  // tuple<fields...>
    class iterator;
    class SplitByTimeDiffOrDistance;

    HighFrequencyTrajectorySplitter(TrajectoryInputIterator start, TrajectoryInputIterator beyond,
            float time_diff_threshold_s, float distance_threshold_m, std::size_t min_points = 2)
            :_traj_it(start), _traj_it_end(beyond), _time_diff_threshold_s(time_diff_threshold_s),
             _distance_threshold_m(distance_threshold_m), _min_points(min_points) { }
    ...
    inline std::optional<trajectory_type> read_trajectory()
    {
        if (_traj_it != _traj_it_end && _split_it == std::end(_splits)) {
            ...
            while(_splits.size() == 0 && _traj_it != _traj_it_end) {
                ...
                Splitter<decltype(split_by_time_diff_or_distance), ProbeInputIterator> hifreq_splitter(_traj_it->begin(),
                        _traj_it->end(), split_by_time_diff_or_distance);
...
public:
    ...
    inline iterator(HighFrequencyTrajectorySplitter& parent)
            :_parent(parent.good() ? &parent : nullptr)
    {
        ++(*this);
    }
