...
public:
	static void register_douglas_peucker(pybind11::module& mod) {
		mod.def("douglas_peucker",
		        [](const typename GeometryModule::Polyline& polyline, NT epsilon) -> typename GeometryModule::Polyline {
			        std::vector<std::decay_t<decltype(polyline.cbegin())>> result;
			        using FindFarthest = movetk::algo::simplification::FindFarthest<MovetkKernel, Norm>;
			        // Apply algorithm
			        movetk::algo::simplification::DouglasPeucker<MovetkKernel, FindFarthest> douglas_peucker_alg(epsilon);
			        douglas_peucker_alg(std::begin(polyline),
			                            std::end(polyline),
	...
	static void register_imai_iri(pybind11::module& mod) {
		mod.def("imai_iri",
		        [](const typename GeometryModule::Polyline& polyline, NT epsilon) -> typename GeometryModule::Polyline {
			        std::vector<std::decay_t<decltype(polyline.cbegin())>> result;
			        // Apply algorithm
			        using ChanChin = movetk::algo::simplification::ChanChin<
			            MovetkKernel,
			            typename GeometryModule::Wedge>;  // Use Chan Chin for constructing viable shortcuts.
			        using ImaiIri = movetk::algo::simplification::ImaiIri<MovetkKernel, ChanChin>;
			        movetk::algo::simplification::ImaiIri<MovetkKernel, ChanChin> imai_iri(epsilon);
	...
	static void register_agarwal(pybind11::module& mod) {
		mod.def("agarwal",
		        [](const typename GeometryModule::Polyline& polyline, NT epsilon) -> typename GeometryModule::Polyline {
			        ...
			        movetk::algo::simplification::Agarwal<MovetkKernel, SqDistance> agarwal(epsilon);
