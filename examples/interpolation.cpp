...
struct InterpolationExampleTraits {
	using NT = typename Kernel::NT;
	struct ProbeTraits {
		enum ProbeColumns { SAMPLE_DATE, LAT, LON, HEADING, SPEED };
		using NT = Kernel::NT;
		using ProbePoint = std::tuple<std::time_t, NT, NT, NT, NT>;
	};
...
struct Example {
	static constexpr const char *NAME = "interpolation";
	static constexpr const char *DESCRIPTION = "Interpolating between points of a trajectory";
	...
	void run(cxxopts::ParseResult &arguments) {
		...
		movetk::io::HighFrequencyTrajectoryReader<Reading::HighFrequencyTrajectoryTraits, false> trajectory_reader(
		    hifreq_time_diff_thr_s,
		    hifreq_dist_diff_thr_m,
		    hifreq_min_num_pts);
		if (arguments.count("file") == 0) {
			throw std::runtime_error("Missing file argument");
		}
		trajectory_reader.init(arguments["file"].as<std::string>().c_str());

		// Create an output csv file
		std::ofstream ofcsv_traj("output_trajectories.csv");

		// Write the header
		movetk::io::print_tuple(ofcsv_traj, trajectory_reader.columns());
		...
		using NT = Kernel::NT;
		typename SegmentationTraits::TSSegmentation segment_by_tdiff(6);
		using InterpolationTraits = InterpolationExampleTraits<Kernel>;
		// Norm to use
		typename InterpolationTraits::Norm norm;
		// Input columns
		constexpr int IN_FILEROW = Reading::ProbeColumns::ORIGFILEROW;
		constexpr int IN_SOURCE = Reading::ProbeColumns::RAW_SOURCE;
		constexpr int IN_DEVICEID = Reading::ProbeColumns::RAW_DEVICEID;
		constexpr int IN_SAMPLE_DATE = Reading::ProbeColumns::RAW_GPSTIME;
		constexpr int IN_LAT = Reading::ProbeColumns::RAW_LAT;
		constexpr int IN_LON = Reading::ProbeColumns::RAW_LON;
		constexpr int IN_HEADING = Reading::ProbeColumns::RAW_HEADING;
		constexpr int IN_SPEED = Reading::ProbeColumns::RAW_SPEED;
		using ProbePoint = typename InterpolationTraits::ProbeTraits::ProbePoint;

		// Output columns
		constexpr int OUT_SAMPLE_DATE = InterpolationTraits::ProbeTraits::ProbeColumns::SAMPLE_DATE;
		constexpr int OUT_LAT = InterpolationTraits::ProbeTraits::ProbeColumns::LAT;
		constexpr int OUT_LON = InterpolationTraits::ProbeTraits::ProbeColumns::LON;
		constexpr int OUT_HEADING = InterpolationTraits::ProbeTraits::ProbeColumns::HEADING;
		constexpr int OUT_SPEED = InterpolationTraits::ProbeTraits::ProbeColumns::SPEED;
		...
		for (const auto &trajectory : trajectory_reader) {
			...
			///
			/// Perform subtrajectory segmentation based on time differences.
			///
			...
			auto lat0 = std::get<IN_LAT>(*ref);
			auto lon0 = std::get<IN_LON>(*ref);
			typename InterpolationTraits::Interpolator interpolator(lat0, lon0);

			///
			/// Interpolate every segment using the defined interpolator
			///
			for (const auto &rows : segment_splitter) {
				// Require segments of size exactly 6
				...
				// Construct the start and end points for the interpolation
				ProbePoint p_u = std::make_tuple(std::get<IN_SAMPLE_DATE>(*first),
				                                 ...
				                                 std::get<IN_SPEED>(*first) * KMPH_TO_METERS_PER_SECOND);

				ProbePoint p_v = std::make_tuple(std::get<IN_SAMPLE_DATE>(*last),
				                                 std::get<IN_LAT>(*last),
				                                 std::get<IN_LON>(*last),
				                                 std::get<IN_HEADING>(*last),
				                                 std::get<IN_SPEED>(*last) * KMPH_TO_METERS_PER_SECOND);
				// Copy timestamps
				std::vector<std::time_t> ts;
				std::transform(first, std::next(last), std::back_inserter(ts), [](const auto &elem) {
					return std::get<IN_SAMPLE_DATE>(elem);
				});
				...
				// Write the input probes, extended with the interpolation
				auto rit = std::begin(interpolated_pts);
				for (const auto &row : rows) {
					// Join the input and output tuples
					std::get<OUT_SPEED>(*rit) = std::get<OUT_SPEED>(*rit) * METERS_PER_SECOND_TO_KMPH;
