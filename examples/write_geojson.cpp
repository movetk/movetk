...
public:
    ...
    void set_property(std::string &key, std::string &val)
    {
        trajectory_properties.push_back(std::make_pair(key, val));
    }

    iterator
    begin()
    {
        return std::cbegin(trajectory_properties);
    }

    iterator
    end()
    {
        return std::cend(trajectory_properties);
    }
    ...
    typename MovetkGeometryKernel::NT get_length()
    {
        return length;
    }

    typename MovetkGeometryKernel::NT get_duration()
    {
        return traj_duration;
    }

    typename MovetkGeometryKernel::NT get_mean_speed()
    {
        return mean_speed;
    }

    typename MovetkGeometryKernel::NT get_median_speed()
    {
        return median_speed;
    }

    typename MovetkGeometryKernel::NT get_min_speed()
    {
        return min_speed;
    }

    typename MovetkGeometryKernel::NT get_max_speed()
    {
        return max_speed;
    }

    typename MovetkGeometryKernel::NT get_var_speed()
    {
        return var_speed;
    }

    typename MovetkGeometryKernel::NT get_time_mode()
    {
        return time_mode;
    }
};
...
void write_geojson(Trajectory_t &trajectory, PropertyIterator first, PropertyIterator beyond,
                   std::ofstream &ofjson)
{
    ...
    std::transform(std::begin(ts), std::end(ts), std::back_inserter(timestamps),
                   [](auto val) -> int { return static_cast<int>(val.ts()); });
...
int main(int argc, char **argv)
{
    ...
    auto tr = TrajectoryReader<TrajectoryTraits, ProbeInputIterator>(probe_reader->begin(), probe_reader->end());
