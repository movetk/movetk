...
struct Example {
	static constexpr const char *NAME = "trajectory_sampler";
	static constexpr const char *DESCRIPTION = R"(<todo>
...
- Input sorted by Timestamp)";
	...
	void run(cxxopts::ParseResult &arguments) {
		...
		std::vector<size_t> columns = arguments["indexes"].as<std::vector<size_t>>();
		...
		using Interpolator = movetk::interpolation::Interpolator<movetk::interpolation::random_trajectory_generator_tag,
		                                                         InterpolationTraits,
		...
		for (; pit != std::cend(trajectory); ++pit) {
			...
			for (auto it = std::begin(interpolated_pts); it != std::end(interpolated_pts) - 1; ++it) {
				auto print_tuple = movetk::utils::sub_tuple<ProbeTraits::ProbeColumns::SAMPLE_DATE,
				                                            ProbeTraits::ProbeColumns::LAT,
				                                            ProbeTraits::ProbeColumns::LON,
				                                            ProbeTraits::ProbeColumns::HEADING,
				                                            ProbeTraits::ProbeColumns::SPEED>(*it);
				std::get<ProbeTraits::ProbeColumns::SPEED>(print_tuple) *= 3.6;
				std::cout << movetk::io::TuplePrinter(print_tuple) << '\n';
			}
		}
	}
};
