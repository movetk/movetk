...
int main(int argc, char** argv) {
	...
	for (auto trajectory : trajectory_reader) {
		...
		if (trajectory.size() == 1) {
			trajectory_geom = movetk::geom::to_point(std::begin(lats), std::begin(lons));
		} else {
			trajectory_geom = movetk::geom::to_linestring(std::begin(lats), std::end(lats), std::begin(lons));
		}
